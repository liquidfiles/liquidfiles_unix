...
void filedrop_command::execute(const cmd::arguments& args)
{
    ...
    if (r) {
//        m_engine.filedrop_attachments(c.server(), c.api_key(), user, subject, message, unnamed_args,
//                rl, c.validate_flag());
    } else {
//        m_engine.filedrop(c.server(), c.api_key(), user, subject, message, unnamed_args,
//                rl, c.validate_flag());
    }
}
